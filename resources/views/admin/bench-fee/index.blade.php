                            ...
                            @foreach ($categories as $category => $rate)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        ...
                                        @if ($rateModel)
                                            <form action="{{ route('admin.bench-fee.destroy', $rateModel) }}" method="POST"
                                                  data-confirm="Hapus tarif {{ $level }} - {{ $type === 'dalam' ? 'Dalam' : 'Luar' }} - {{ $category === 'biomedis' ? 'Biomedis' : 'Non-Biomedis' }}?" data-confirm-accept="Ya, Hapus">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                        class="rounded-lg bg-red-50 p-1.5 text-red-600 transition hover:bg-red-100"

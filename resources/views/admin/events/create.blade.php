...
@section('title', 'Buat Event - MarketLabs')

@section('page', 'Buat Event')

@section('content')

@php
    $initialForm = json_decode(old('form_fields', '[]'), true) ?: [];
    $initialAttendance = json_decode(old('attendance_fields', '[]'), true) ?: [];
@endphp

<div class="flex flex-wrap items-center justify-between gap-4">
    <div>
        <h1 class="text-2xl font-extrabold tracking-tight text-slate-900">Buat Event Baru</h1>
        <p class="mt-1 text-sm text-slate-600">Lengkapi detail event, form registrasi, dan form presensi.</p>
    </div>
...
<form method="POST" action="{{ route('admin.events.store') }}" enctype="multipart/form-data" class="mt-8 space-y-6">
    @csrf
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-7">
        ...
        <div class="mt-5 grid gap-4 md:grid-cols-2">
            ...
            <div>
                ...
                <input type="number" id="quota" name="quota" value="{{ old('quota') }}" min="1"
            ...
            <div>
                ...
                <select id="status" name="status" required
                    ...
                    @foreach ([
                        ...
                        <option value="{{ $value }}" {{ old('status', \App\Models\Event::STATUS_DRAFT) === $value ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <select id="mode" name="mode"
                    ...
                    @foreach (\App\Models\Event::modes() as $value => $label)
                        <option value="{{ $value }}" {{ old('mode') === $value ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <p class="mt-1 text-xs text-slate-400">Gambar kecil untuk kartu event di daftar. PNG, JPG, WEBP maks. 5 MB.</p>
            </div>
            <div>
                <label for="poster" class="block text-sm font-semibold text-slate-600">Poster Event</label>
                <input type="file" id="poster" name="poster" accept="image/*"
                       class="mt-1 w-full cursor-pointer rounded-lg border border-slate-300 bg-white text-xs text-slate-600 file:mr-2 file:cursor-pointer file:rounded-lg file:border-0 file:bg-emerald-600 file:px-3 file:py-1.5 file:text-xs file:font-semibold file:text-white hover:file:bg-emerald-700">
                <p class="mt-1 text-xs text-slate-400">Gambar besar untuk halaman detail event. Disarankan potret (poster). Maks. 10 MB.</p>
            </div>
            <div class="md:col-span-2">
                <label for="description" class="block text-sm font-semibold text-slate-600">Deskripsi</label>
                <textarea id="description" name="description" rows="5"
                          class="mt-1 w-full rounded-lg border border-slate-300 px-4 py-2.5 text-sm text-slate-900 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/30">{{ old('description') }}</textarea>
            </div>
        </div>
    </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-7">
        <div class="flex items-center justify-between">
            ...
            <label class="relative inline-flex cursor-pointer items-center">
                <input type="checkbox" name="attendance_enabled" value="1" {{ old('attendance_enabled', 1) ? 'checked' : '' }}
        ...
        <div id="attendance-fields-wrap" class="{{ old('attendance_enabled', 1) ? '' : 'hidden' }}">
            @include('admin.events._form_builder', [

        ...
        <table class="min-w-full divide-y divide-slate-200">
            <thead class="bg-slate-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Jumlah Alat</th>
            ...
            <tbody class="divide-y divide-slate-200">
                @forelse ($categories as $i => $category)
                    <tr class="transition hover:bg-slate-50">
                        ...
                        @endif
                        <td class="whitespace-nowrap px-6 py-4 text-sm text-slate-500">{{ $categories->firstItem() + $i }}</td>
                        <td class="px-6 py-4 text-sm font-medium text-slate-900">{{ $category->name }}</td>
                        <td class="px-6 py-4 text-sm text-slate-600">{{ $category->tools_count }} alat</td>
                        <td class="px-6 py-4">
                    ...
                    <tr id="edit-form-{{ $category->id }}" class="hidden">
                        <td colspan="{{ auth()->user()->isSuperAdmin() ? 5 : 4 }}" class="bg-emerald-50/50 px-6 py-4">
                            <form action="{{ route('admin.categories.update', $category) }}" method="POST" class="flex gap-3">
                                @csrf
                                @method('PUT')
                                <input type="text" name="name" value="{{ $category->name }}" required
                                       class="w-full max-w-md rounded-lg border border-slate-300 px-4 py-2.5 text-sm text-slate-900 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/30">
                                <button type="submit"
                ...
                @empty
                    <tr>
                        <td colspan="{{ auth()->user()->isSuperAdmin() ? 5 : 4 }}" class="px-6 py-10 text-center text-sm text-slate-500">Belum ada kategori.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>

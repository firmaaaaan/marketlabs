...
@section('title', 'Edit Laboratorium - MarketLabs')
...
<h1 class="mt-3 text-2xl font-extrabold tracking-tight text-slate-900">Edit Laboratorium: {{ $laboratorium->name }}</h1>
...
<form action="{{ route('admin.laboratoriums.update', $laboratorium) }}" method="POST" class="mt-8 max-w-2xl space-y-4">
    @csrf
    @method('PUT')

    <div>
        <label for="name" class="block text-sm font-semibold text-slate-700">Nama Laboratorium <span class="text-red-500">*</span></label>
        <input type="text" id="name" name="name" value="{{ old('name', $laboratorium->name) }}" required maxlength="255"
    ...
    <div>
        ...
        <input type="text" id="code" name="code" value="{{ old('code', $laboratorium->code) }}" maxlength="50"
               class="mt-1.5 w-full rounded-lg border border-slate-300 px-4 py-2.5 text-sm text-slate-900 placeholder-slate-400 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/30">
    </div>
    ...
    <div>
        ...
        <textarea id="description" name="description" rows="4" maxlength="2000"
                  class="mt-1.5 w-full rounded-lg border border-slate-300 px-4 py-2.5 text-sm text-slate-900 placeholder-slate-400 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/30">{{ old('description', $laboratorium->description) }}</textarea>
    </div>
    ...
    <label class="flex cursor-pointer items-center justify-between gap-4 rounded-xl border border-slate-200 bg-white px-4 py-3.5">
        ...
        <input type="checkbox" name="is_active" value="1" {{ $laboratorium->is_active ? 'checked' : '' }} class="peer sr-only">

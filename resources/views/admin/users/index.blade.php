                            ...
                            <div class="flex justify-end gap-2">
                                @if ($user->id !== auth()->id())
                                    <form action="{{ route('admin.users.reset-password', $user) }}" method="POST"
                                          data-confirm="Reset password {{ $user->name }} ke default?" data-confirm-accept="Ya, Reset">
                                        @csrf
                                        <button type="submit"
                                                class="rounded-lg bg-amber-50 p-1.5 text-amber-600 transition hover:bg-amber-100"

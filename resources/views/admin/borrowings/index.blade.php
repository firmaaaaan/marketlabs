...
<div id="bulk-action-bar" class="mt-6 hidden rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3">
    <div class="flex flex-wrap items-center gap-3">
        ...
        <input type="text" id="bulk-rejection-reason" placeholder="Alasan penolakan (jika ditolak)"
               class="hidden w-full max-w-xs rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/30">
...
<form id="bulk-status-form" action="{{ route('admin.borrowings.bulk-status') }}" method="POST">
    @csrf
    @method('PATCH')
            ...
            <tbody class="divide-y divide-slate-200">
                @forelse ($borrowings as $borrowing)
                    <tr class="transition hover:bg-slate-50">
                        <td class="px-4 py-4 text-center"><input type="checkbox" class="bulk-checkbox rounded border-slate-300 text-emerald-600 focus:ring-emerald-500" value="{{ $borrowing->id }}" onchange="updateBulkBar()"></td>
                ...
                @empty
                    <tr>
                        <td colspan="8" class="px-6 py-10 text-center text-sm text-slate-500">Belum ada peminjaman.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
function submitBulkStatus() {
    ...
    if (checked.length === 0) { alert('Pilih minimal satu peminjaman.'); return; }

    const ids = Array.from(checked).map(cb => cb.value);
    document.getElementById('bulk-ids').value = JSON.stringify(ids);
    document.getElementById('bulk-status-input').value = status;

    const reasonInput = document.getElementById('bulk-rejection-reason');
    document.getElementById('bulk-rejection-input').value = status === 'rejected' ? reasonInput.value : '';

    if (status === 'rejected' && !reasonInput.value) {
...
document.getElementById('bulk-status').addEventListener('change', function() {
    const reasonInput = document.getElementById('bulk-rejection-reason');
    reasonInput.classList.toggle('hidden', this.value !== 'rejected');
});
</script>

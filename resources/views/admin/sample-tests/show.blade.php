...
<div class="mt-6 flex flex-wrap items-center gap-3">
    ...
    <div class="flex items-center gap-2">
        ...
        <a href="{{ route('admin.sample-tests.edit', $test) }}"
           class="rounded-lg bg-slate-100 px-4 py-2 text-xs font-semibold text-slate-700 transition hover:bg-emerald-100 hover:text-emerald-700">
            Edit
        </a>
        <form action="{{ route('admin.sample-tests.destroy', $test) }}" method="POST"
            ...
            @method('DELETE')
            <button type="submit"
                    class="rounded-lg bg-red-50 px-4 py-2 text-xs font-semibold text-red-600 transition hover:bg-red-100">
                Hapus
            </button>
        </form>
    </div>

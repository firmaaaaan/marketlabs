...
@section('title', 'Buat Pengujian Sampel - MarketLabs')
...
<h1 class="mt-4 text-2xl font-extrabold tracking-tight text-slate-900">Buat Pengujian Sampel</h1>
<p class="mt-1 text-sm text-slate-600">Buat pengajuan pengujian untuk pemohon secara manual. Satu layanan boleh digunakan untuk banyak sampel.</p>
...
<form action="{{ route('admin.sample-tests.store') }}" method="POST" class="mt-8 space-y-6">
    @csrf
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div class="mt-5 grid gap-5 sm:grid-cols-2">
            <div class="sm:col-span-2">
                ...
                @if ($users->isEmpty())
                    <p class="mt-1 text-xs text-amber-600">Belum ada user berperan pemohon.</p>
                @endif
            </div>
            <div class="sm:col-span-2">
                ...
                <textarea id="notes" name="notes" rows="2"
                          class="mt-1.5 w-full rounded-lg border border-slate-300 px-4 py-2.5 text-sm text-slate-900 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/30">{{ old('notes') }}</textarea>
            </div>
        </div>
    </div>
    ...
    <div class="space-y-6">
        ...
        @empty
            <div class="rounded-2xl border border-slate-200 bg-white px-6 py-12 text-center shadow-sm">
                <p class="text-sm text-slate-500">Belum ada layanan pengujian aktif. Tambahkan lewat Kelola Parameter.</p>
            </div>
        @endforelse
    </div>
...
<script>
    ...
    // Toggle visibilitas daftar sampel berdasarkan checkbox "sertakan layanan".

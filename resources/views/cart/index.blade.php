                                ...
                                @foreach ($items as $item)
                                    <div class="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center">
                                        ...
                                        <div class="flex items-center gap-4">
                                            ...
                                            {{-- Ubah jumlah --}}
                                            <form action="{{ route('cart.update', $item['tool']) }}" method="POST" class="flex items-center gap-2" data-auto-submit>
                                                @csrf
                                                @method('PATCH')
...
@push('scripts')
<script>
document.querySelectorAll('[data-auto-submit] input[name="quantity"]').forEach(function (input) {
    let debounce;

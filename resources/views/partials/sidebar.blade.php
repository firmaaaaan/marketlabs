<aside id="sidebar"
    ...
    <div class="flex items-center justify-between px-6 py-5">
        ...
        {{-- Static menu: Analitik --}}
        @php
            $analyticsActive = request()->routeIs('admin.analytics*');
        @endphp
        <a href="{{ route('admin.analytics') }}"
           class="flex items-center gap-3 rounded-lg px-4 py-2.5 text-sm font-medium transition {{ $analyticsActive ? 'bg-white/10 text-white font-semibold' : 'text-emerald-100 hover:bg-white/10 hover:text-white' }}">
        ...
        @forelse ($sidebarMenuItems as $item)
            @php
                ...
                $group = match(true) {
                    str_contains($item->route_name ?? '', 'dashboard') || str_contains($item->route_name ?? '', 'notif') || str_contains($item->route_name ?? '', 'calendar') || str_contains($item->route_name ?? '', 'kalender') || str_contains($item->route_name ?? '', 'analytics') => 'Umum',
            ...
            <a href="{{ $itemRoute }}"
               class="flex items-center gap-3 rounded-lg px-4 py-2.5 text-sm font-medium transition {{ $isActive ? 'bg-white/10 text-white font-semibold' : 'text-emerald-100 hover:bg-white/10 hover:text-white' }}">
                    ...
                    $iconPath = $item->icon ?? match(true) {
                        ...
                        str_contains($item->route_name ?? '', 'analytics') => 'M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 013 19.875v-6.75zM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V8.625zM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V4.125z',
                        ...
                        str_contains($item->route_name ?? '', 'calendar') || str_contains($item->route_name ?? '', 'kalender') => 'M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5m-9-6h.008v.008H12v-.008zM12 15h.008v.008H12V15zm0 2.25h.008v.008H12v-.008zM9.75 15h.008v.008H9.75V15zm0 2.25h.008v.008H9.75v-.008zM7.5 15h.008v.008H7.5V15zm0 2.25h.008v.008H7.5v-.008zm6.75-4.5h.008v.008h-.008v-.008zm0 2.25h.008v.008h-.008V15zm0 2.25h.008v.008h-.008v-.008zm2.25-4.5h.008v.008H16.5v-.008zm0 2.25h.008v.008H16.5V15z',
                        ...
                        str_contains($item->route_name ?? '', 'maintenance') => 'M11.42 15.17L17.25 21A2.652 2.652 0 0021 17.25l-5.877-5.877M11.42 15.17l2.496-3.03c.317-.384.74-.626 1.208-.766M11.42 15.17l-4.655 5.653a2.548 2.548 0 11-3.586-3.586l6.837-5.63m5.108-.233c.55-.164 1.163-.188 1.743-.14a4.5 4.5 0 004.486-6.336l-3.276 3.277a3.004 3.004 0 01-2.25-2.25l3.276-3.276a4.5 4.5 0 00-6.336 4.486c.091 1.076-.071 2.264-.904 2.95l-.102.085m-1.745 1.437L5.909 7.5H4.5L2.25 3.75l1.5-1.5L7.5 4.5v1.409l4.26 4.26m-1.745 1.437l1.745-1.437m6.615 8.206L15.75 15.75M4.867 19.125h.008v.008h-.008v-.008z',
                        str_contains($item->route_name ?? '', 'backup') => 'M20.25 6.375c0 2.278-3.694 4.125-8.25 4.125S3.75 8.653 3.75 6.375m16.5 0c0-2.278-3.694-4.125-8.25-4.125S3.75 4.097 3.75 6.375m16.5 0v11.25c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125V6.375m16.5 0v3.75m-16.5-3.75v3.75m16.5 0v3.75C20.25 16.153 16.556 18 12 18s-8.25-1.847-8.25-4.125v-3.75m16.5 0c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125',
    ...
    <div class="border-t border-white/10 px-4 py-4 space-y-1">
